...
export async function PUT(
  ...
  try {
    ...
    try {
      // Clear cache for old slug if slug changed
      if (validatedData.slug !== existingPage.slug) {
        await clearCache(`page:${existingPage.slug}`);
      }
      // Clear cache for new slug
      await clearCache(`page:${validatedData.slug}`);
      console.log('[Cache] Cleared page cache after update');
    } catch (cacheError) {
      console.error('[Cache] Failed to clear cache after page update:', cacheError);
...
export async function DELETE(
  ...
  try {
    ...
    try {
      await clearCache(`page:${existingPage.slug}`);
      console.log('[Cache] Cleared page cache after deletion');
    } catch (cacheError) {
      console.error('[Cache] Failed to clear cache after page deletion:', cacheError);
      // Don't fail the request if cache clearing fails
    }
