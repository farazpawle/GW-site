...
import { clearPattern, clearCache } from '@/lib/cache';
...
export async function PUT(
  ...
  try {
    ...
    try {
      ...
      await clearCache(`products:detail:${id}`);
      console.log('[Cache] Cleared product caches after update');
    } catch (cacheError) {
      console.error('[Cache] Failed to clear cache after product update:', cacheError);
...
export async function DELETE(
  ...
  try {
    ...
    try {
      await clearPattern('products:*');
      await clearCache(`products:detail:${id}`);
      console.log('[Cache] Cleared product caches after deletion');
    } catch (cacheError) {
      console.error('[Cache] Failed to clear cache after product deletion:', cacheError);
      // Don't fail the request if cache clearing fails
    }
