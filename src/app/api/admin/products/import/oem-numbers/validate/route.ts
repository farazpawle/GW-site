...
/**
 * POST /api/admin/products/import/oem-numbers/validate
 * Validate OEM Numbers CSV file before import
 */
export async function POST(request: NextRequest) {
  try {
    ...
    records.forEach((record, index) => {
      ...
      const { errors, warnings } = validateOEMNumberCSVRow(
        row,
        rowIndex,
        existingSKUs
      );
    ...
    const preview = records.slice(0, 10).map((record, index) => ({
      rowNumber: index + 2,
      data: parseOEMNumberCSVRow(record),
    }));
    ...
    console.error('Error validating OEM numbers import:', error);
