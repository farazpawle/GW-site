...
import { 
  parseOEMNumberCSVRow, 
  validateOEMNumberCSVRow, 
  transformCSVToOEMNumber,
  ValidationError 
} from '@/lib/csv-utils';

/**
 * POST /api/admin/products/import/oem-numbers/execute
 * Execute OEM Numbers CSV import
 * 
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      records.forEach(record => {
        const row = parseOEMNumberCSVRow(record);
      ...
      // Process each product's OEM numbers
      for (const [sku, productRecords] of recordsByProduct) {
        ...
        // Delete existing OEM numbers for this product
        await tx.oEMPartNumber.deleteMany({
          where: { partId },
        });
        ...
        for (let i = 0; i < productRecords.length; i++) {
          ...
          try {
            ...
            const { errors: validationErrors } = validateOEMNumberCSVRow(
              row,
              rowIndex,
              existingSKUs
            );
            ...
            const oemData = transformCSVToOEMNumber(row, partId);
            await tx.oEMPartNumber.create({
              data: oemData,
            });
            created++;
    ...
    console.error('Error executing OEM numbers import:', error);
