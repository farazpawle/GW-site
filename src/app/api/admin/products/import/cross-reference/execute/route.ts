...
import { 
  parseCrossReferenceCSVRow, 
  validateCrossReferenceCSVRow, 
  transformCSVToCrossReference,
  ValidationError 
} from '@/lib/csv-utils';

/**
 * POST /api/admin/products/import/cross-reference/execute
 * Execute Cross Reference CSV import
 * 
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      records.forEach(record => {
        const row = parseCrossReferenceCSVRow(record);
      ...
      // Process each product's cross references
      for (const [sku, productRecords] of recordsByProduct) {
        ...
        // Delete existing cross references for this product
        await tx.partCrossReference.deleteMany({
          where: { partId },
        });
        ...
        for (let i = 0; i < productRecords.length; i++) {
          ...
          try {
            ...
            const { errors: validationErrors } = validateCrossReferenceCSVRow(
              row,
              rowIndex,
              existingSKUs
            );
            ...
            const crossRefData = transformCSVToCrossReference(row, partId);
            await tx.partCrossReference.create({
              data: crossRefData,
            });
            created++;
    ...
    console.error('Error executing cross reference import:', error);
