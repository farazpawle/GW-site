...
/**
 * POST /api/admin/products/import/cross-reference/validate
 * Validate Cross Reference CSV file before import
 */
export async function POST(request: NextRequest) {
  try {
    ...
    records.forEach((record, index) => {
      ...
      const { errors, warnings } = validateCrossReferenceCSVRow(
        row,
        rowIndex,
        existingSKUs
      );
    ...
    const preview = records.slice(0, 10).map((record, index) => ({
      rowNumber: index + 2,
      data: parseCrossReferenceCSVRow(record),
    }));
    ...
    console.error('Error validating cross reference import:', error);
