...
export async function PUT(
  ...
  try {
    const user = await checkPermission('categories.edit');
    if (!user) {
      return NextResponse.json(
        { success: false, error: 'Unauthorized' },
        { status: 401 }
    ...
    try {
      await clearPattern('categories:*');
      await clearPattern('products:*');
      console.log('[Cache] Cleared category and product caches after category update');
    } catch (cacheError) {
      console.error('[Cache] Failed to clear cache after category update:', cacheError);
...
export async function DELETE(
  ...
  try {
    const user = await checkPermission('categories.delete');
    if (!user) {
      return NextResponse.json(
        { success: false, error: 'Unauthorized' },
        { status: 401 }
    ...
    try {
      await clearPattern('categories:*');
      await clearPattern('products:*');
      console.log('[Cache] Cleared category and product caches after category deletion');
    } catch (cacheError) {
      console.error('[Cache] Failed to clear cache after category deletion:', cacheError);
      // Don't fail the request if cache clearing fails
    }
