...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      candidateUrls.push(`${protocol}//${fallbackHost}:${fallbackPort}${urlObj.pathname}${urlObj.search}`);
      if (fallbackHost !== 'minio') {
        candidateUrls.push(`${protocol}//minio:${fallbackPort}${urlObj.pathname}${urlObj.search}`);
      }
    } catch (parseError) {
