...
export const brandStorySectionConfigSchema = z.object({
  ...
  content: z
    .array(z.string().min(1, "Content paragraph cannot be empty"))
...
export type BrandStorySectionConfig = z.infer<
  typeof brandStorySectionConfigSchema
>;
...
export const carouselSectionConfigSchema = z.object({
  ...
  itemsPerView: z
    ...
    .optional()
    .default({ mobile: 2, tablet: 3, desktop: 5 }),
  logos: z
    .array(
      z.object({
...
export const categoriesSectionConfigSchema = z.object({
  ...
  cardStyle: z
    .enum(["boxed", "minimal", "image-heavy"])
    .optional()
    .default("boxed"),
  iconPosition: z
...
export type CategoriesSectionConfig = z.infer<
  typeof categoriesSectionConfigSchema
>;
...
export const precisionMfgSectionConfigSchema = z.object({
  ...
  cardStyle: z
    .enum(["standard", "minimal", "image-heavy", "side-by-side"])
    .optional(),
...
export type PrecisionMfgSectionConfig = z.infer<
  typeof precisionMfgSectionConfigSchema
>;
...
export const updatePageSectionSchema = z.object({
  sectionType: z
    .enum(["hero", "brandStory", "carousel", "categories", "precisionMfg"])
    .optional(),
...
export function isHeroConfig(
  config: SectionConfig,
): config is HeroSectionConfig {
  return "badge" in config && "title" in config && "description" in config;
}

export function isBrandStoryConfig(
  config: SectionConfig,
): config is BrandStorySectionConfig {
  return (
    "features" in config &&
    Array.isArray((config as BrandStorySectionConfig).features)
  );
}

export function isCarouselConfig(
  config: SectionConfig,
): config is CarouselSectionConfig {
  return (
    "logos" in config && Array.isArray((config as CarouselSectionConfig).logos)
  );
}

export function isCategoriesConfig(
  config: SectionConfig,
): config is CategoriesSectionConfig {
  return (
    "categories" in config &&
    Array.isArray((config as CategoriesSectionConfig).categories)
  );
}

export function isPrecisionMfgConfig(
  config: SectionConfig,
): config is PrecisionMfgSectionConfig {
  return (
    "services" in config &&
    Array.isArray((config as PrecisionMfgSectionConfig).services)
  );
}
