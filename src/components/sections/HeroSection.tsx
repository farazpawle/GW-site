...
function InteractiveChar({
  ...
  useEffect(() => {
    ...
    if (distance < radius) {
      ...
      const r = Math.round(
        hoverColorStart[0] +
          intensity * (hoverColorEnd[0] - hoverColorStart[0]),
      );
      const g = Math.round(
        hoverColorStart[1] +
          intensity * (hoverColorEnd[1] - hoverColorStart[1]),
      );
      const b = Math.round(
        hoverColorStart[2] +
          intensity * (hoverColorEnd[2] - hoverColorStart[2]),
      );
      setColor(`rgb(${r}, ${g}, ${b})`);
    } else {
      setColor(baseColor);
          ...
          >
            {config.description.split("").map((char, index) => (
              <InteractiveChar
                ...
                mouseY={mousePosition.y}
                baseColor={
                  config.descriptionStyle?.color || "rgb(209, 213, 219)"
                }
