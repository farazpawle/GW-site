...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Pointer interface for fluid simulation
    interface Pointer {
      id: number;
      texcoordX: number;
      texcoordY: number;
    ...
    // WebGL uniform locations mapped by name
    ...
    const blit = (() => {
      ...
      return (target: any, clear = false) => {
        if (target == null) {
          ...
          const fbo = target.fbo || target.read.fbo;
          gl.bindFramebuffer(gl.FRAMEBUFFER, fbo);
        }
        if (clear) {
          gl.clearColor(0.0, 0.0, 0.0, 1.0);
    ...
    // FBO objects for fluid simulation - complex WebGL types
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    function initFramebuffers() {
      ...
      // Guard against null formats
      if (!rgba || !rg || !r) {
        console.error('WebGL texture formats not supported');
        return;
      }
