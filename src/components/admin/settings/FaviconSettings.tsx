 ...
 */
export default function FaviconSettings({ formData, onChange }: FaviconSettingsProps) {
  const [previews, setPreviews] = useState<Record<string, string>>({});
  const [errors, setErrors] = useState<Record<string, string>>({});

  const faviconFields = [
    { key: 'favicon_ico', label: 'Favicon (.ico)', size: '16x16 or 32x32', accept: '.ico' },
    { key: 'favicon_16', label: 'PNG Icon 16x16', size: '16x16', accept: 'image/png' },
    { key: 'favicon_32', label: 'PNG Icon 32x32', size: '32x32', accept: 'image/png' },
    { key: 'favicon_192', label: 'Android Icon', size: '192x192', accept: 'image/png' },
    { key: 'apple_touch_icon', label: 'Apple Touch Icon', size: '180x180', accept: 'image/png' },
  ];
  ...
  const handleFileUpload = async (key: string, file: File) => {
    ...
    if (!validTypes.includes(file.type)) {
      setErrors({ ...errors, [key]: 'Invalid file type. Use PNG or ICO format.' });
      return;
    }
  ...
  return (
    <div className="space-y-6">
      {/* Favicon Section */}
      <div>
        <h3 className="text-lg font-semibold text-white mb-4">Favicon & App Icons</h3>
        <p className="text-sm text-gray-400 mb-6">
          Upload favicons and app icons for different platforms and sizes. These icons appear in browser tabs, bookmarks, and on mobile home screens.
        </p>

        <div className="space-y-4">
          {faviconFields.map((field) => (
            <div key={field.key} className="bg-gray-800/50 p-4 rounded-lg border border-gray-700">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">
                    {field.label}
                  </label>
                  <p className="text-xs text-gray-500">Recommended size: {field.size}</p>
                </div>
              ...
              {previews[field.key] ? (
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    <div className="w-16 h-16 bg-white rounded-lg flex items-center justify-center p-2 border border-gray-300">
                      ...
                      <img
                        ...
                        onError={(e) => {
                          console.error('Failed to load favicon preview:', field.key, previews[field.key]);
                          ...
                          if (parent && !parent.querySelector('.error-placeholder')) {
                            ...
                            errorDiv.className = 'error-placeholder text-xs text-red-500 text-center';
                            errorDiv.textContent = '✗';
                            parent.appendChild(errorDiv);
                          }
                        }}
                        onLoad={() => {
                          console.log('Successfully loaded favicon:', field.key);
                        }}
                      />
                    </div>
                  ...
                  <div className="text-xs text-gray-500 truncate max-w-md" title={previews[field.key]}>
                    URL: {previews[field.key].substring(0, 60)}...
                  </div>
                </div>
              ) : (
                <div className="text-gray-500 text-sm mb-3">No icon uploaded</div>
              )}
      ...
      <div className="p-4 bg-blue-900/20 border border-blue-700/30 rounded-lg">
        ...
        <ul className="text-xs text-blue-300/90 space-y-1 ml-5 list-disc">
          ...
          <li>Apple Touch Icon appears when users save your site to iPhone home screen</li>
