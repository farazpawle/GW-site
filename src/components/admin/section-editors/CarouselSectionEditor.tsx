...
export default function CarouselSectionEditor({ section, isOpen, onClose, onSave }: CarouselSectionEditorProps) {
  ...
  const migrateConfig = (oldConfig: any): CarouselSectionConfig => {
    return {
      ...
      itemsPerView: oldConfig.itemsPerView || { mobile: 2, tablet: 3, desktop: 5 },
  ...
  const [config, setConfig] = useState<CarouselSectionConfig>(migrateConfig(section.config));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const cleanedConfig = {
        ...
        itemsPerView: config.itemsPerView || { mobile: 2, tablet: 3, desktop: 5 },
  ...
  const updateLogo = (index: number, field: 'description' | 'altText' | 'image' | 'isActive', value: string | boolean) => {
  ...
  const handleMediaSelect = (file: MediaFile) => {
    if (selectedLogoIndex !== null) {
      updateLogo(selectedLogoIndex, 'image', file.url);
    }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Brand Carousel Section" size="xl">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4 p-4 bg-[#0a0a0a] rounded-lg border border-[#2a2a2a]">
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">Leave empty to use default name: &quot;Brand Carousel&quot;</p>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="mt-2">
              <TypographyControls
                ...
                onChange={(headingStyle) => setConfig({ ...config, headingStyle })}
              />
            </div>
          </div>
        ...
        {/* Carousel Settings */}
        <div className="space-y-4 p-4 bg-[#0a0a0a] rounded-lg border border-[#2a2a2a]">
          <h3 className="text-lg font-semibold text-white">Carousel Settings</h3>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Mobile (Items)</label>
              <input
                type="number"
                min="1"
                max="4"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Tablet (Items)</label>
              <input
                type="number"
                min="2"
                max="6"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Desktop (Items)</label>
              <input
                type="number"
                min="3"
                max="8"
        ...
        <div className="space-y-4">
          ...
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
            <p className="text-sm text-blue-400">
              💡 <strong>Tip:</strong> You can use image URLs from Unsplash or upload images to Media Library first, then paste the URL here.
            </p>
          </div>
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-gray-400">Logo {index + 1}</span>
                  <span className={`text-xs px-2 py-1 rounded ${
                    logo.isActive ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'
                  }`}>
                ...
                <div className="flex items-center gap-2">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => updateLogo(index, 'isActive', e.target.checked)}
