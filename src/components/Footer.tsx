  ...
  if (privacyPolicyPageId || termsPageId) {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/pages`,
        {
          cache: "no-store",
        },
      );
      if (response.ok) {
        ...
        if (privacyPolicyPageId) {
          const privacyPage = pages.find(
            (p: { id: string }) => p.id === privacyPolicyPageId,
          );
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li>
                <Link
                  href="/about"
                  className="text-gray-400 hover:text-[#ff9999] transition-colors flex items-center gap-2 group"
                >
          ...
          <div>
            ...
            <ul className="space-y-3">
              ...
              <li>
                <Link
                  href="/products?difficulty=Beginner"
                  className="text-gray-400 hover:text-[#ff9999] transition-colors flex items-center gap-2 group"
                >
                  <span className="w-1.5 h-1.5 rounded-full bg-gray-600 group-hover:bg-[#6e0000] transition-colors"></span>
                  Easy Install Parts
        ...
        <div className="border-t border-[#2a2a2a] mt-8 pt-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-sm">
            ...
            <div className="flex items-center gap-2 px-4 py-2 rounded-lg border border-[#2a2a2a] bg-[#1a1a1a]/50">
              ...
              <a
                href="https://web.whatsapp.com/send?phone=+971555975965"
                target="_blank"
                rel="noopener noreferrer"
                className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#ff9999] to-[#6e0000] tracking-wider hover:opacity-80 transition-opacity"
              >
                FRZ
              </a>
            </div>
