...
export function validateEnv(): Env {
  ...
  if (process.env.CI === "true") {
    console.log(
      "⚠️  CI environment detected - using mock environment variables for build",
    );
    return {
      ...
      NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY:
        "pk_test_Y2ktdGVzdC1rZXkuZXhhbXBsZS5jb20k",
...
/**
 * Validated environment variables (lazy initialized).
 * Import this object instead of accessing process.env directly
 * to get type-safe environment variables.
 *
 * Validation is deferred until first access to allow CI builds.
 */
